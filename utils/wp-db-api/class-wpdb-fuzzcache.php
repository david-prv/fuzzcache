...
class wpdb
{
    ...
    {
        $protected_members = array('col_meta', 'table_charset', 'check_current_query', 'allow_unsafe_unquoted_parameters');
    ...
    public function set_charset($dbh, $charset = null, $collate = null)
    {
        ...
        if ($this->has_cap('collation') && !empty($charset)) {
            ...
            if ($set_charset_succeeded) {
                ...
                PHPSHMCache\sqlWrapperFunc('mysqli_query', array($dbh, $query));
            }
        }
    }
    /**
    ...
    {
        if (empty($modes)) {
            $res = PHPSHMCache\sqlWrapperFunc('mysqli_query', array($this->dbh, 'SELECT @@SESSION.sql_mode'));
            if (empty($res)) {
                return;
            }
            $modes_array = PHPSHMCache\sqlWrapperFunc('mysqli_fetch_array', array($res));
        ...
        PHPSHMCache\sqlWrapperFunc('mysqli_query', array($this->dbh, "SET SESSION sql_mode='{$modes_str}'"));
    }
    /**
    ...
    public function _real_escape($data)
    {
        ...
        if ($this->dbh) {
            $escaped = PHPSHMCache\sqlWrapperFunc('mysqli_real_escape_string', array($this->dbh, $data));
        } else {
    ...
    public function prepare($query, ...$args)
    {
        ...
        if (false === strpos($query, '%')) {
            wp_load_translations_early();
            _doing_it_wrong('wpdb::prepare', sprintf(
        ...
        while ($key < $split_query_count) {
            ...
            if ('f' === $type && true === $this->allow_unsafe_unquoted_parameters && '%' === substr($split_query[$key - 1], -1, 1)) {
                /*
                ...
                if ('i' === $type) {
                     ...
                     */
                    if (true !== $this->allow_unsafe_unquoted_parameters || '' === $format && '%' !== substr($split_query[$key - 1], -1, 1)) {
                        $placeholder = "'%" . $format . "s'";
                    }
                }
            }
    ...
    public function print_error($str = '')
    {
        global $EZSQL_ERROR;
        if (!$str) {
            $str = PHPSHMCache\sqlWrapperFunc('mysqli_error', array($this->dbh));
        }
    ...
    public function check_connection($allow_bail = true)
    {
        ...
        if (!empty($this->dbh) && PHPSHMCache\sqlWrapperFunc('mysqli_query', array($this->dbh, 'DO 1')) !== false) {
            return true;
        }
    ...
    public function query($query)
    {
        ...
        // If there is an error then take note of it.
        if ($this->dbh instanceof mysqli) {
            $this->last_error = PHPSHMCache\sqlWrapperFunc('mysqli_error', array($this->dbh));
        } else {
    ...
    private function _do_query($query)
    {
        ...
        if (!empty($this->dbh)) {
            $this->result = PHPSHMCache\sqlWrapperFunc('mysqli_query', array($this->dbh, $query));
        }
        ++$this->num_queries;
        if (defined('SAVEQUERIES') && SAVEQUERIES) {
    ...
    public function log_query($query, $query_time, $query_callstack, $query_start, $query_data)
    {
        ...
        $this->queries[] = array($query, $query_time, $query_callstack, $query_start, $query_data);
    }
    /**
    ...
    protected function check_safe_collation($query)
    {
        ...
        $safe_collations = array('utf8_bin', 'utf8_general_ci', 'utf8mb3_bin', 'utf8mb3_general_ci', 'utf8mb4_bin', 'utf8mb4_general_ci');
    ...
    public function bail($message, $error_code = '500')
    {
        if ($this->show_errors) {
            $error = '';
            if ($this->dbh instanceof mysqli) {
                $error = PHPSHMCache\sqlWrapperFunc('mysqli_error', array($this->dbh));
            } elseif (mysqli_connect_errno()) {
                $error = PHPSHMCache\sqlWrapperFunc('mysqli_connect_error', array());
            }
            if ($error) {
    ...
    public function close()
    {
        ...
        $closed = PHPSHMCache\sqlWrapperFunc('mysqli_close', array($this->dbh));
        if ($closed) {
            $this->dbh = null;
            $this->ready = false;
    ...
    public function has_cap($db_cap)
    {
         ...
         * the polyfills from wp-includes/compat.php are not loaded.
         */
        if ('5.5.5' === $db_version && false !== strpos($db_server_info, 'MariaDB') && PHP_VERSION_ID < 80016) {
